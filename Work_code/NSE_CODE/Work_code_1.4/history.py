...
def is_consolidating(niftydf, percentage_range=20, period=10):
    # Calculate the highest high and lowest low in the period
    niftydf['max_high'] = niftydf['high'].rolling(window=period).max()
    niftydf['min_low'] = niftydf['low'].rolling(window=period).min()

    # Calculate the percentage range of the price
    niftydf['range'] = (niftydf['max_high'] - niftydf['min_low']) / niftydf['min_low'] * 100

    # If the range is within the given percentage, it is consolidating
    niftydf['is_consolidating'] = niftydf['range'] <= percentage_range

    return niftydf

# Step 6: Check ATR (Average True Range) to measure volatility
def check_atr(niftydf, period=14):
    niftydf['ATR'] = talib.ATR(niftydf['high'], niftydf['low'], niftydf['close'], timeperiod=period)
    return niftydf


# Step 7: Consolidation criteria
def check_market_consolidation(niftydf):
    niftydf = check_atr(niftydf)  # Add ATR for volatility measurement
    niftydf = is_consolidating(niftydf)  # Check if it's within the consolidation range

    # Consolidation signal when true for last period
    latest_consolidation = niftydf['is_consolidating'].iloc[-1]
...
if not niftydf.empty:
    consolidation_status = check_market_consolidation(niftydf)
...
niftydf['is_consolidating'] = niftydf['is_consolidating'].astype(str)
signals_df = niftydf[['EMA_Signal', 'RSI_Signal', 'Supertrend_Signal','close','is_consolidating']]
