...
# 4. Sell stock if signal is "Sell" and no existing order
def sell_stock(symbol, order_file='order.json', signal_file='signal.json'):
    supertrend_signal = check_signal(signal_file)
    existing_order = check_existing_order(symbol, order_file)
    if supertrend_signal == "Sell":
        if not existing_order:
            logging.info(f"Placing sell order for {symbol}.")
            place_new_order(symbol, order_type="Sell", order_file=order_file)
...
def main():
    ...
    if supertrend_signal == "Buy":
        order_id = buy_stock(symbol)  # Get the order ID fromm buy_stock
