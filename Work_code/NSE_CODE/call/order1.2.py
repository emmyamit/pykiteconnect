...
with open('.././core/kite_key.json', 'r') as f:
    keys = json.load(f)
...
with open('.././core/kite_session.json', 'r') as f:
    ...
    if os.path.exists(signal_file):
        with open(signal_file, 'r') as file:
            try:
                ...
                print("Error reading signal.json")
                return None
    else:
        print(f"{signal_file} not found!")
        return None
    ...
    if supertrend_signal == "Buy":
        if not existing_order or existing_order['order_type'] == "Sell":
            ...
            try:
                ...
                print(f"Buy order placement failed: {e}")
        else:
            print(f"Buy order for {symbol} already exists. Waiting for sell signal.")
        ...
        if not existing_order or existing_order['order_type'] == "Buy":
            ...
            print(f"Sell order for {symbol} already exists. Waiting for new buy signal.")

    else:
        print(f"No actionable signal for {symbol}: {supertrend_signal}")
...
def main():
    print("Starting the order management process.")
